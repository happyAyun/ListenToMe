...
export default {
    ...
    video: function () {
      return document.querySelector('#user-video:nth-of-type(1) > video')  // 내담자 비디오 영역 선택
    },
	},	
        ...
        .catch(errors => {
          console.log(errors)  // 모델을 로딩하는 과정에서 문제가 발생하면 에러 출력
				...
				setInterval(async () => {
					...
					const width = detections[0].landmarks.imageWidth * 0.7
					const height = detections[0].landmarks.imageHeight * 0.7
					const pointX = detections[0].detection.box.x - width * 1.2
					const pointY = detections[0].detection.box.y - height / 1.8
